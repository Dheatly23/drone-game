...
func _process(delta: float) -> void:
	...
	var res: Dictionary = __level.level_query.query_ray(
		position,
...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action("release_capture"):
		Input.mouse_mode = Input.MOUSE_MODE_CAPTURED if event.is_released() else Input.MOUSE_MODE_VISIBLE
